...
const Dashboard = () => {
    ...
    const dashboardCards = [
        ...
        { title: "Active Shows", value: ((dashboardData.activeShows?.length || 0) + (dashboardData.activeNightlifeEvents?.length || 0) + (dashboardData.activeSportEvents?.length || 0)) || "0", icon: PlayCircleIcon },
    ...
    <>
      ...
      <div className="relative flex flex-wrap gap-4 mt-6">
            ...
            <div className="relative flex flex-wrap gap-6 mt-4 max-w-5xl">
                ...
                {/* Nightlife Events */}
                {dashboardData.activeNightlifeEvents && dashboardData.activeNightlifeEvents.map((event) => (
                    <div key={event._id} className="w-55 rounded-lg overflow-hidden h-full pb-3 bg-primary/10 border border-primary/20 hover:-translate-y-1 transition duration-300">
                        ...
                        <div className="flex items-center justify-between px-2">
                            ...
                            <p className="text-sm text-gray-400 mt-1 pr-1">{event.category}</p>
                        </div>
                        <p className="px-2 pt-2 text-sm text-gray-500">{dateFormat(event.showDateTime)}</p>
                    </div>
                ))}

                {/* Sport Events */}
                {dashboardData.activeSportEvents && dashboardData.activeSportEvents.map((event) => (
                    <div key={event._id} className="w-55 rounded-lg overflow-hidden h-full pb-3 bg-primary/10 border border-primary/20 hover:-translate-y-1 transition duration-300">
                        <img src={event.image} alt={event.title} className="h-60 w-full object-cover" />
                        <p className="font-medium p-2 truncate">{event.title}</p>
                        <div className="flex items-center justify-between px-2">
                            <p className="text-lg font-medium">{currency} {event.price}</p>
                            <p className="text-sm text-gray-400 mt-1 pr-1">{event.category}</p>
                        </div>
                        <p className="px-2 pt-2 text-sm text-gray-500">{dateFormat(event.date)}</p>
                    </div>
                ))}
            </div>
